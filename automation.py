...
class PoetryPushTask(AutomationTask):
    ...
    def run(self):
        ...
        # 使用 MiMo API 生成内容
        prompt = f"""
请推荐一首冷门但有文学价值的古诗（第{self.current_poem}首），要求：
1. 不要推荐耳熟能详的作品（如静夜思、登鹳雀楼、春晓、悯农、游子吟等小学必背古诗）
2. 优先选择唐宋诗词中较少被提及但艺术成就较高的作品
3. 可以选择一些小众诗人的佳作，或名家的冷门作品
4. 避免选择过于生僻难懂的作品，保持一定的可读性
